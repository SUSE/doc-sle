...
XML_SOURCE_FILES := $(wildcard xml/*.xml)
...
MO_FILES := $(filter .$(LANGS).,$(addsuffix .mo,$(basename $(PO_FILES))))

DC_SOURCE_FILES := $(wildcard DC-*)

# If LANGS is not defined, for output, use only those files that have at least 60% translations
XML_DEST_FILES := $(foreach LANG, $(LANGS), $(LANG)/xml/$(notdir $(XML_SOURCE_FILES)))
...
MO_FILE = $(subst xml,po,$(@D))/$(addsuffix .mo,$(basename $(@F)).$(subst /xml,,$(@D)))
